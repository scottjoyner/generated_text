#!/usr/bin/env python3
"""
Build a clean, sortable CSV of core Hugging Face model metadata from cached model JSONs.

Inputs:
  - models_enriched.csv  (columns: model_name, url, model_description, params, model_size, [optional] updated_url)
  - cache/               (contains HF /api/models JSON files; supports both "<org__repo>.json" and "model_<org__repo>.json")

Output:
  - model_metadata.csv   (one row per unique repo_id with core, sortable metadata)

Deduping rule:
  - Prefer rows that have a valid canonical HF URL (updated_url if present, else url).
  - Fallback to any row that resolves to a repo_id if no clean URL rows exist.

“Relevant core metadata” (siblings excluded) includes:
  repo_id, canonical_url, name_hf_slug, model_name (original), author, pipeline_tag,
  library_name, license, languages, tags, downloads, likes, created_at, last_modified,
  private, gated, disabled, sha, model_type, architectures, auto_model, processor,
  parameters (int), parameters_readable, used_storage_bytes, region, arxiv_ids, and
  the enrichment columns (model_description, params_raw, model_size_raw).

Usage:
  python build_model_metadata.py \
      --input models_enriched.csv \
      --cache cache \
...
def is_clean_hf_url(url: str) -> bool:
    try:
        ...
        if not url.startswith("https://huggingface.co/"):
            return False
        # Path should be "<repo>" or "<org>/<repo>" (no extra segments like /tree,/resolve)
...
def best_repo_id_for_row(row: pd.Series) -> Optional[str]:
    """
    Prefer updated_url (if present and clean), then url (if clean),
    else model_name if it already looks like org/repo or single-tenant repo.
    """
    ...
    # Last resort: model_name contains "/" (e.g., "Cohere/Cohere-embed-english-v3.0")
...
def choose_preferred_row(rows: pd.DataFrame) -> pd.Series:
    """
    From a group of rows representing the same repo_id, choose the one with a clean HF URL.
    """
    # prefer rows with updated_url clean
    candidates = rows[rows["updated_url_clean"] == True]
    if len(candidates) > 0:
        return candidates.iloc[0]
    # then rows with url clean
...
def cache_json_for_repo(cache_dir: Path, repo_id: str) -> Optional[dict]:
    """
    Load cached JSON for repo_id, supporting both naming patterns:
      - "<org__repo>.json"
      - "model_<org__repo>.json"
    """
...
def parse_params_to_int(*vals) -> Optional[int]:
    ...
    Parse parameters from:
      - safetensors.total (int)
      - string like '14m', '7B', '1.3b', '567755777'
      Returns int or None.
    """
    ...
    for v in vals:
        ...
        if isinstance(v, int):
            return v
        # JSON may deliver floats even for large ints
...
def parse_metadata(repo_id: str, js: dict, enrich_row: pd.Series) -> Dict[str, object]:
    ...
    # Tags (merge top-level + cardData.tags)
    top_tags = list_from(js.get("tags"))
    card_tags = list_from(js.get("cardData", {}).get("tags"))
    tags = list(dict.fromkeys(top_tags + card_tags))  # unique, preserve order
...
def main():
    ...
    # Add flags for dedupe preference (handle missing columns safely)
    ...
    # Make sure columns are in a nice order
    col_order = [
